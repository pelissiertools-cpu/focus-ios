...
class ProjectsViewModel: ObservableObject, TaskEditingViewModel, LogFilterable {
    ...
    func toggleProjectPin(_ project: FocusTask) async {
        let newPinned = !project.isPinned
        do {
            try await repository.togglePin(id: project.id, isPinned: newPinned)
    ...
    func togglePin(_ task: FocusTask, projectId: UUID) async {
    ...
    func requestToggleTaskCompletion(_ task: FocusTask, projectId: UUID) {
        ...
        pendingCompletion.scheduleCompletion(for: taskId, action: { [weak self] in
            guard let self,
                  let tasks = self.projectTasksMap[projectId],
